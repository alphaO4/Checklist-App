...
import { setupProtocolHandler, getPendingDeepLink, clearPendingDeepLink } from './deeplink';
...
app.whenReady().then(async () => {
  // Setup custom protocol handler for deep links
  ...
  // Handle pending deep link after app starts
  const pendingDeepLink = getPendingDeepLink();
  if (pendingDeepLink) {
    // Wait for window to be ready, then process deep link
    mainWindow?.webContents.once('did-finish-load', () => {
      mainWindow?.webContents.send('deep-link-received', { url: pendingDeepLink });
